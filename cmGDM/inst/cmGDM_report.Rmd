...
```{r echo=FALSE}
#impScores <- round(100*params$thisExperiment$model$varImp[[2]][,1]/sum(params$thisExperiment$model$varImp[[2]][, 1]), 2)
impScores <- round(params$thisExperiment$model$varImp[[2]][,1], 3)
...
varImpTable <- matrix(c(paddedNames, paddedNums), ncol = 2, dimnames = list(NULL, c("Covariate", "Contribution")))
...
The second plot shows observed compositional dissimilarity _versus_ predicted compositional dissimlarity and is expected to be clear linear relationship indicated by the light blue line. A good model will show a tight, fairly even distribution of points around the line. As before, note that points on this plot represent values of _dissimilarity between pairs of sites_. 


```{r echo=FALSE, fig.align="center", fig.height=4, fig.width=4, warning=FALSE}

#![](`r paste0(path.expand(outputPath), "/cmGDM_", params$thisExperiment$experimentName , "_GDM_observed_predicted_dissimilarity.png")`)
...
The gdm software provides a number of methods to evaluating variable importance. The following table shows the simplest measure, the _change_ in deviance for a GDM fitted with the variable removed. A large value indicates a large reduction in GDM deviance when the variable was omitted. A value at or close to zero indicates a covariate which may be safely discarded.

`r knitr::kable(varImpTable, align = c("r", "r")) `
